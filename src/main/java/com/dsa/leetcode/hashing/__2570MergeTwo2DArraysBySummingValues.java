...
public class __2570MergeTwo2DArraysBySummingValues {
    public int[][] mergeArrays(int[][] nums1, int[][] nums2) {

        Map<Integer, Integer> map = Arrays.stream(nums1)
                .filter(pair -> pair != null && pair.length == 2)
                .collect(Collectors.toMap(
                        pair -> pair[0],
                        pair -> pair[1],
                        (oldValue, newValue) -> oldValue, // Handle duplicate keys
                        TreeMap::new // Use TreeMap to automatically sort by keys
                ));


        Arrays.stream(nums2).forEach(pair -> map.put(pair[0], map.getOrDefault(pair[0], 0) + pair[1]));
    ...
    public int[][] mergeArraysUsing2Pointers(int[][] nums1, int[][] nums2) {//Merge Sort kinda logic
